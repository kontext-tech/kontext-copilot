...
from kontext_copilot.ollama._types import Message, Options, RequestError, ResponseError
...
class BaseClient:
    def __init__(
        ...
        **kwargs,
        ...
        headers["User-Agent"] = (
            f"ollama-python/{__version__} ({platform.machine()} {platform.system().lower()}) Python/{platform.python_version()}"
        )
...
class Client(BaseClient):
    ...
    def chat(
        ...
        for message in messages or []:
            ...
            if not (role := message.get("role")) or role not in [
                "system",
                "user",
                "assistant",
            ]:
                raise RequestError(
                    'messages must contain a role and it must be one of "system", "user", or "assistant"'
                )
    ...
    def create(
        ...
        if (realpath := _as_path(path)) and realpath.exists():
            modelfile = self._parse_modelfile(
                realpath.read_text(), base=realpath.parent
            )
        elif modelfile:
...
class AsyncClient(BaseClient):
    ...
    async def _parse_modelfile(
        self, modelfile: str, base: Optional[Path] = None
    ) -> str:
        base = Path.cwd() if base is None else base

<template>
    <BDropdown variant="outline-secondary">
        ...
        <BDropdownItem v-if="error">{{ error }}</BDropdownItem>
        <BDropdownItem v-for="(ds, index) in dataSources" :key="ds.id" @click="selectSource(ds)">
            <div class="d-flex align-items-center cursor-pointer">
                <span>{{ ds.name }}</span>
                <BBadge variant="light" class="ms-1">{{ ds.type }}</BBadge>
                <Icon v-if="selectedDataSource?.id === ds.id" name="material-symbols:check"
